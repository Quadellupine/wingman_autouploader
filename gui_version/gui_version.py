...
def on_moved(event):
    ...
    if event.dest_path not in seen_files and (event.dest_path.endswith(".zevtc")):
        ...
        if "WvW/" in event.dest_path:
            window.start_thread(
                lambda: upload(event.dest_path, True), ("-THREAD-", "-THEAD ENDED-")
            )
        else:
            window.start_thread(
                lambda: upload(event.dest_path, no_wingman),
                ("-THREAD-", "-THEAD ENDED-"),
            )
    ...
    while True:
        ...
        # Open additional Window to start a batch upload, handled in additional python file
            ...
            try:
                ...
                sg.popup_notify(
                    "Copied!", display_duration_in_ms=1000, fade_in_duration=0, alpha=1
                )
            except:
                pass
